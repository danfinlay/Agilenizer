...
Identity.prototype.logTask = function (task) {
  ...
  var newRatio = task.stats.ratio
  var oldRatio = record.avg
  var ratioChange = newRatio - oldRatio

  // Velocity Average
  if ('velocity' in record) {

    console.log("First velocity computing for:")
    console.dir(record)
    var lastAvg = record.avg
    var newAvg = ((lastAvg * examples) + newRatio) / this.record[key].examples
    record.velocity.last = newAvg - record.velocity.last
    record.velocity.avg = newAvg
  } else {
    console.log("Initializing velocity with %s %s and %s", newRatio, oldRatio, ratioChange)
    this.record[key].velocity = {
      avg: ratioChange,
      last: ratioChange
    }
  }
