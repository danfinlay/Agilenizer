...
Identity.prototype.logTask = function (task) {
  ...
  if (!this.record[key]) {
    this.record[key] = {
      examples: 0,
      avg: 0,
      first: task.stats.ratio,
      last: task.stats.ratio
    }
  }
  ...
  var record = this.record[key]
  this.logVelocity(task, record)
}

Identity.prototype.logVelocity = function (task, record) {
  var newRatio = record.avg
  var firstRatio = record.first
  var ratioChange = task.stats.ratio - record.last
  ...
  if (!('velocity' in record)) {
    ...
    // How much better they get on average:
    record.velocity.avg = (task.stats.ratio - firstRatio) / (record.examples - 1)

    // How much better they got this last time:
    record.velocity.last = task.stats.ratio - record.last
    record.last = task.stats.ratio
  }
}
...
Identity.prototype.remove = function (completed, opts) {
  var recordKey = this.recordKeyForQuery(completed);
...
Identity.prototype.velocityFor = function (query) {
  var recordKey = this.recordKeyForQuery(query)
  var task = this.record[recordKey]  
  return task.velocity
}

Identity.prototype.recordKeyForQuery = function (query) {
  var taskArr = taskArrayize.bind(this)(query || '')
  return taskArr.join()
}
